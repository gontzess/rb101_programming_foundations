...
def find_dup(ary)
  ...
  dup = nil
  ary.each do |value|
    temp_store.include?(value) ? dup = value : temp_store[value] += 1
  end
  dup
end

def find_dup_vLS(ary)
  ary.find { |element| ary.count(element) == 2 }
end

def find_dup_vLS2(ary)
  ary.tally.each { |k, v| return k if v == 2 }
end
...
p find_dup_vLS([1, 5, 3, 1])
puts find_dup_vLS([1, 5, 3, 1]) == 1
puts find_dup_vLS([18,  9, 36, 96, 31, 19, 54, 75, 42, 15,
          38, 25, 97, 92, 46, 69, 91, 59, 53, 27,
          14, 61, 90, 81,  8, 63, 95, 99, 30, 65,
          78, 76, 48, 16, 93, 77, 52, 49, 37, 29,
          89, 10, 84,  1, 47, 68, 12, 33, 86, 60,
